...
window               WINDOW('LPC3250 NOR Flash Loader'),AT(,,341,188),FONT('Arial',8,,,CHARSET:ANSI),ICON('CLARION.ICO'),TIMER(1),SYSTEM,GRAY,IMM
                       ...
                       GROUP('Download Progress'),AT(4,160,331,24),USE(?Progress),BOXED
                         ENTRY(@N9),AT(255,169,50,10),USE(n),SKIP,HIDE,RIGHT(1),COLOR(COLOR:BTNFACE),READONLY
                         BUTTON('Create test.bin'),AT(278,165,49,14),USE(?Button3),HIDE
                         PROGRESS,USE(?Progress1),AT(11,171,315,8),RANGE(0,100)
                       END
                     END
...
ReturnValue          BYTE,AUTO
  ...
  window{PROP:Text} = 'LPC3250 NOR Flash Loader - ' & com
    ...
    OF EVENT:Timer
      ...
      IF retcode NOT= -1 THEN
        ...
        OF 0
          ...
          ComPutc(port, VAL('A'))
          state=1
          q:line = 'Waiting for BootID...Found!  Sent char A'
          PUT(list)
          q:line = 'Waiting for BootID...'
          ADD(list)
        OF 1
          ...
          ComPutc(port, VAL('3'))
          state=2
          q:line = 'Waiting for BootID...Found!  Sent chars U3'
          PUT(list)
        ...
        OF 4
          ...
          LOOP
            ...
            END
            n += 1
            ?Progress1{PROP:Progress} = (n / size) * 100
            DISPLAY
            IF index =32768 THEN               
              index = 1
              RETURN 0
