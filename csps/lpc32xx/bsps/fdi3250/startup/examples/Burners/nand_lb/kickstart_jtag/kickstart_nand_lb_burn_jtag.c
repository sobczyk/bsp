/***********************************************************************
 * $Id:: kickstart_nand_lb_burn.c 3415 2010-05-06 19:52:29Z usb10132  $
 *
...
void c_entry(void)
{
  ...
  memset(BURNER_LOAD_ADDR, 0xCC, BURNER_LOAD_SIZE);

  /* Now to download the image */
  uart_output((UNS_8 *)"Load Kickstart into memory using JTAG.\r\n");

  // Wait for the end of the stage1 load area to be written to
  while (1) {
      temp = *((volatile UNS_32 *)(BURNER_LOAD_ADDR));
      if (temp != 0xCCCCCCCC)
          break;
  }

  uart_output((UNS_8 *)"Calculating Kickstart size ...\r\n");

  // Look for a word that is not 0xCCCCCCCC
  for (idx=BURNER_LOAD_SIZE-8; idx>=0; idx-=4) {
      temp = *((volatile UNS_32 *)(BURNER_LOAD_ADDR+idx));
      if (temp != 0xCCCCCCCC)
          break;
  }
  ...
  uart_output((UNS_8 *)"Burning Kickstart ... \r\n");
  ...
  if (nand_lb_mlc_erase_block(progblk) == 0)
  {
	...
	uart_output((UNS_8*)"Erase failure (MLC failure, bad block?)\r\n");
  ...
  if(idx >= 0)
  {
		uart_output((UNS_8*)"Erase failure (not 0xFF's)\r\n");
		nand_lb_mlc_write_sector(sector, wrbuff8);
		while (1);
  }
