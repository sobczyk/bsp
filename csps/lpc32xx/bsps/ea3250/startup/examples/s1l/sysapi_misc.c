/***********************************************************************
 * $Id:: sysapi_misc.c 3431 2010-05-07 17:39:37Z usb10132              $
 *
...
BOOL_32 cmd_vcore(void);
static UNS_32 cmd_vcore_plist[] =
{
	PARSE_TYPE_STR, /* The "vcore" command */
	(PARSE_TYPE_DEC | PARSE_TYPE_END)
};
static CMD_ROUTE_T core_vcore_cmd =
{
	(UNS_8 *) "vcore",
	cmd_vcore,
	(UNS_8 *) "Sets val3250 vcore voltage",
	(UNS_8 *) "vcore <value in mV, 1350 max>",
	cmd_vcore_plist,
	NULL
};
...
#ifdef ENABLE_DEBUG
/* rosci command */
BOOL_32 cmd_rosci(void);
static UNS_32 cmd_rosci_plist[] =
{
	(PARSE_TYPE_STR | PARSE_TYPE_END) /* The "rosci" command */
};
static CMD_ROUTE_T core_rosci_cmd =
{
	(UNS_8 *) "rosci",
	cmd_rosci,
	(UNS_8 *) "Runs ring oscillator calibration range checks",
	(UNS_8 *) "rosci",
	cmd_rosci_plist,
	NULL
};
 ...
 * Function: cmd_rosci
 *
 * Purpose: Runs ring oscillator tests
 *
 * Processing:
 *     Performs some testing of the ring oscillator for core voltage
 *     supply stability determination. For DDR systems, this test can
 *     help determine if the power supply has problems.
 *
 * Parameters: None
 *
 * Outputs: None
 ...
 *
 * Notes:
 *     The caches should be enabled for these tests.
 *
 **********************************************************************/
BOOL_32 cmd_rosci(void) 
{
	int idx;
	UNS_32 min_rosc, max_rosc, tmp32;
	UNS_8 str[32];

	min_rosc = max_rosc = CLKPWR->clkpwr_ddr_lap_count;
	for (idx = 0; idx < 10000; idx++)
	{
		/* Force calibration cycle */
		CLKPWR->clkpwr_sdramclk_ctrl |= CLKPWR_SDRCLK_DO_CAL;
		CLKPWR->clkpwr_sdramclk_ctrl &= ~CLKPWR_SDRCLK_DO_CAL;
		timer_wait_us(TIMER_CNTR1, 50);

		tmp32 = CLKPWR->clkpwr_ddr_lap_count;
		if (tmp32 > max_rosc)
		{
			max_rosc = tmp32;
		}
		else if (tmp32 < min_rosc)
		{
			min_rosc = tmp32;
		}
	}

	term_dat_out((UNS_8 *) "Min / max ring oscillator counts: ");
	str_makedec(str, min_rosc);
	term_dat_out(str);
	term_dat_out((UNS_8 *) " / ");
	str_makedec(str, max_rosc);
	term_dat_out_crlf(str);
	term_dat_out((UNS_8 *) "Current nominal count: ");
	str_makedec(str, CLKPWR->clkpwr_ddr_lap_nom);
	term_dat_out(str);
...
BOOL_32 cmd_clock(void) 
{
	...
	if (processed == TRUE) 
	{
		...
		else
		{
			...
			/* Reset the board for the new clocking to take
			   effect */
			board_reset();
		}
	}
 ...
 **********************************************************************/
 BOOL_32 cmd_vcore(void) {
	...
	tst = cmd_get_field_val(1);
	ea3250_vcore_set(tst);
...
void ucmd_init(void)
{
...
#ifdef ENABLE_DEBUG
	cmd_add_new_command(&core_group, &core_rosci_cmd);
	...
	cmd_add_new_command(&core_group, &core_vcore_cmd);
...
#ifdef ENABLE_CKLSWITCHING
	/* Do the DDR init once more here at the current system speed or
	   DDR may still be configured for the default speed. */
	cp15_dcache_flush();
...
void cmd_info_extend(void)
{
	...
	term_dat_out_crlf(str);
	term_dat_out((UNS_8 *) "32-bit bus             : ");
	str_makehex(str, SDRAM_32BIT_BUS, 1);
	term_dat_out_crlf(str);
	...
	term_dat_out((UNS_8 *) "Uncalibrated DQS min   : ");
	str_makehex(str, dqsstart, 8);
	term_dat_out_crlf(str);
	term_dat_out((UNS_8 *) "Uncalibrated DQS max   : ");
	str_makehex(str, dqsend, 8);
	term_dat_out_crlf(str);
#endif
}
