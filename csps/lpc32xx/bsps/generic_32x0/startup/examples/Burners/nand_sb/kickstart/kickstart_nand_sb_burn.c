...
static UNS_32 rdbuff[528 / 4], wrbuff[528 / 4];
...
void nand_write_sectors(int sector, UNS_8 *wrbuffer, int size)
{
		while (size > 0) 
		{
			nand_sb_mlc_write_sector(sector, wrbuffer);
...
int verify_data(int sector, UNS_8 *wrbuffer, UNS_8 *rdbuffer, int size)
{
	int idx = 0;
		while (size > 0) 
		{
			nand_sb_mlc_read_sector(sector, rdbuffer);
